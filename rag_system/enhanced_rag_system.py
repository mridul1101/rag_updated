...
#                     # Check if any documents were retrieved at all
#                     retrieved_docs = rag_result.get('sources', [])
                    
#                     if not retrieved_docs:
#                         logger.info("No documents retrieved from vector store - falling back to web search")
#                     else:
#                         # If we have documents, check their relevance
#                         relevance_score = self._evaluate_relevance(query_text, retrieved_docs)
#                         logger.info(f"RAG relevance score: {relevance_score:.2f} (threshold: {self.confidence_threshold})")
...
#                 # Step 2: If RAG failed (no docs or low confidence), try web search
#                 logger.info("Falling back to web search")
#                 web_result = self.web_search_agent.query(query_text)
                
#                 # Add provenance information
#                 if not force_web_search and retrieved_docs:
#                     # Only show this message if we had some docs but they weren't good enough
#                     web_result["answer"] = "I couldn't find enough relevant information in your documents, so I searched the web:\n\n" + web_result["answer"]
...
from config.config import EMBEDDING_MODEL, INDEX_PATH, MODEL_NAME, logger
from document_processing.document_processor import DocumentProcessor
from retrieval.retriever import ContentFilteredRetriever
from web_search.web_search_agent import WebSearchAgent
...
class EnhancedRAGSystem:
    def __init__(self, api_key: str, model_name: str = MODEL_NAME, 
                 embedding_model: str = EMBEDDING_MODEL,
                 index_path: str = INDEX_PATH,
                 confidence_threshold: float = 0.7):  # Higher default threshold
        """Initialize the enhanced RAG system with web search capabilities."""
        os.environ["GOOGLE_API_KEY"] = api_key
        self.api_key = api_key
        self.confidence_threshold = confidence_threshold
        ...
        logger.info(f"Enhanced RAG System initialized with model {model_name}")
    ...
    def create_vector_store(self, documents: List[Document]) -> None:
        ...
        try:
            ...
            logger.info(f"Vector store created with {len(documents)} docs in {time.time() - start_time:.2f}s")
        except Exception as e:
            logger.error(f"Error creating vector store: {str(e)}")
            raise

    def create_rag_chain(self, chain_type: str = "stuff", k: int = 4) -> None:
        """Create a RAG chain with content filtering."""
        if not self.vector_store:
            raise ValueError("Vector store not initialized")
            
        logger.info(f"Creating RAG chain with {k} documents per query")
        retriever = ContentFilteredRetriever(vector_store=self.vector_store, k=k)
        self.rag_chain = RetrievalQA.from_chain_type(
            llm=self.llm,
            chain_type=chain_type,
            retriever=retriever,
    ...
    def _evaluate_relevance(self, query: str, sources: List[Dict[str, Any]]) -> float:
        """Strict relevance evaluation based on content matching."""
        if not sources:
            return 0.0
        ...
        try:
            ...
            for source in sources:
                ...
                # Immediate fail if negative phrases found
                if any(phrase in content for phrase in ["no information", "not contain", "does not mention"]):
                    return 0.0
                
                # Check for query term matches
                content_terms = set(content.split())
                matches = query_terms & content_terms
                if len(matches) >= max(1, len(query_terms)//2):
                    relevant_count += 1
            
            score = relevant_count / len(sources)
            logger.info(f"Relevance score: {score:.2f} ({relevant_count}/{len(sources)} relevant)")
            return score
    ...
    def _execute_rag_query(self, query: str) -> Dict[str, Any]:
    ...
    # def query(self, query_text: str, force_web_search: bool = False) -> Dict[str, Any]:
    #     """Main query method with automatic web fallback."""
    #     logger.info(f"Processing query: {query_text}")
        
    #     # Direct web search if forced
    #     if force_web_search:
    #         return self.web_search_agent.query(query_text)
        
    #     # Try RAG first
    #     rag_result = self._execute_rag_query(query_text)
        
    #     # Conditions for web search fallback:
    #     # 1. No sources found, OR
    #     # 2. Answer contains negative phrases, OR
    #     # 3. Low relevance score
    #     if (not rag_result["sources"] 
    #         or any(phrase in rag_result["answer"].lower() 
    #               for phrase in ["no information", "not found", "don't know"])
    #         or self._evaluate_relevance(query_text, rag_result["sources"]) < self.confidence_threshold):
            
    #         logger.info("Falling back to web search")
    #         web_result = self.web_search_agent.query(query_text)
            
    #         # Add context about why web was used
    #         if not rag_result["sources"]:
    #             web_result["answer"] = "This information wasn't in your documents:\n\n" + web_result["answer"]
    #         else:
    #             web_result["answer"] = "I couldn't find clear information in your documents:\n\n" + web_result["answer"]
    ...
    def query(self, query_text: str, force_web_search: bool = False) -> Dict[str, Any]:
        """Improved query method with guaranteed web fallback when docs don't help"""
        logger.info(f"Processing query: {query_text}")
        
        # Short-circuit to web if forced
        if force_web_search:
            return self.web_search_agent.query(query_text)
        ...
        # Define clear negative answer patterns
        NEGATIVE_PHRASES = [
            "no information",
            "not contain",
            "does not mention",
        ...
        use_web = (
            not rag_result["sources"] or  # No docs found
            any(phrase in answer_lower for phrase in NEGATIVE_PHRASES) or  # Negative answer
            not any(term in answer_lower for term in query_lower.split()[:3])  # Answer doesn't contain query terms
        )
        
        if use_web:
            logger.info("Insufficient document results - using web search")
            web_result = self.web_search_agent.query(query_text)
            
            # Explain why we used web results
            if not rag_result["sources"]:
                prefix = "This information wasn't in your documents"
            elif "sorry" in answer_lower or "i cannot" in answer_lower:
                prefix = "My documents couldn't answer this clearly"
            else:
                prefix = "I couldn't find complete information in your documents"
            
            web_result["answer"] = f"{prefix}:\n\n{web_result['answer']}"
            return web_result
